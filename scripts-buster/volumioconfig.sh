...
echo "     is qemu-user-static 2.1 in jessie too old for this???"
...
if [ $(uname -m) = armv7l ] || [ $(uname -m) = aarch64 ]; then
  ...
  if [ $ARCH = arm ]; then
	 echo "===> amr/armv7 not ready for buster!!"
...
wpa_passphrase=volumio2" >> /etc/hostapd/hostapd-edimax.conf
  ...
  wget http://repo.volumio.org/Volumio2/Buster/Custom%20Packages/mpd_0.21.16-1_i386.deb
...
# Shairport-Sync is in the Debian buster repo
  ...
  wget http://repo.volumio.org/Volumio2/Buster/Custom%20Packages/volumio-remote-updater_1.5-i386.deb
