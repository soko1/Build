...
if [ -n "$BUILD" ]; then
  ...
  if [ "$BUILD" = arm ] || [ "$BUILD" = arm-dev ]; then
    ...
    echo "Unexpected Base System architecture '$SUITE $BUILD' - aborting."
    exit
  fi
...
case "$DEVICE" in
  ...
  vim1) echo 'Writing VIM1 Image File'
    check_os_release "armv7" "$VERSION" "$DEVICE"
    sh ${BSCRIPTS}/vim1image.sh -v "$VERSION" -p "$PATCH" -a armv7
    ;;
  kvim1|kvim2|kvim3|kvim3l) echo 'Writing Khadas $DEVICE Image File'
    check_os_release "armv7" "$VERSION" "$DEVICE"
